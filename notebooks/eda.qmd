...
# dataset
hourly_df <- py$hour_of_day_metrics_py |>
  dplyr::mutate(
    hour_posix = as.POSIXct(
      sprintf("1900-01-01 %02d:00:00", hour),
      format = "%Y-%m-%d %H:%M:%S",
      tz = "PST"
    )
  )
...
# background
# Define regions: 
# Region 1: x from 0 to 7.5, color 'lightgreen'
# Region 2: x from 7.5 to 16, color 'salmon'
rects <- data.frame(ystart = seq(39,70,181), yend = seq(69,180,400), col = c("High", "In Range","Low"))


# plot
ggplot() +
  geom_rect(data = rects, aes(ymin = ystart, ymax = yend, xmin = -Inf, xmax = Inf, fill = col), alpha = 0.4) +
  geom_line(hourly_df, aes(x = hour_posix, y = mean)) +
  scale_x_datetime(date_labels = "%I:%M %p") +
  scale_y_continuous(limits = c(39, 300),
