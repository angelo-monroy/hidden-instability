...
```{r}
# Bring Python df into R
hourly_dow_df <- as.data.frame(py$hour_dow_metrics_py)

# Ensure types are what we want
hourly_dow_df <- hourly_dow_df |>
  mutate(
    hour = as.integer(hour),
    # Pretty hour label like "12:00 PM"
    hour_label = format(
      as.POSIXct(
...
ggplot(hourly_dow_df,
       aes(x = hour_label, y = mean, group = 1)) +
  geom_rect(
    data = rects,
    aes(ymin = ystart, ymax = yend, xmin = -Inf, xmax = Inf, fill = Region),
    inherit.aes = FALSE,
  ...
  labs(
    x = "Hour of Day",
    y = "Mean Blood Glucose Mg/dL",
    title = "Mean Glucose by Hour, by Day of Week"
  ) +
   theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))
```
