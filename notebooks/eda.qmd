...
Tables and figures in this section and below come from `notebooks/tables/` and `notebooks/images/`. Generate them by running from the project root (with your venv activated):

```bash
python scripts/build_eda_data.py
```

(Optional: pass a CSV path if not using `data/raw/*.csv`.)
